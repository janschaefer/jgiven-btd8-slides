...
public class ThenOrder extends Stage<ThenOrder> {
    ...
    public ThenOrder a_corresponding_order_exists_for_the_customer() {
    ...
    public ThenOrder there_are_$_items_left_on_stock(int numberOfItems) {
        StockItem stockItem = stockRepository.getByBook(books.get(books.size() - 1));
        assertThat(stockItem.count).isEqualTo(numberOfItems);
        return this;
    }

    public ThenOrder no_corresponding_order_exists_for_the_customer() {
        List<Order> orders = orderRepository.getOrdersOfCustomer(customer);
    ...
    public ThenOrder the_stock_looks_as_follows(@Table List<GivenBook.BookOnStock> stock) {
    ...
    public ThenOrder the_stock_looks_as_follows(@Table String[][] stock) {
